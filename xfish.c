...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void msgdie(message) char *message;
{
  fprintf(stderr, "%s: %s\n", pname, message);
  exit(1);
}
  ...
  if (f->frame) {
    ...
    if (pfishA[f->type][f->d]) {
      ...
      if (DoClipping) {
        ...
        XCopyArea(Dpy, pfishA[f->type][f->d], wid, gc, 0, 0, rwidth[f->type],
                  rheight[f->type], f->x, f->y);
      } else if (picname[0] != '\0') {
        gcv.fill_style = FillTiled;
        gcv.tile = PicMap;
        gcv.ts_x_origin = -(f->x);
        ...
        XChangeGC(Dpy, pgc,
                  ...
                  &gcv);
        XFillRectangle(Dpy, PixBuf, pgc, 0, 0, rwidth[f->type],
                       rheight[f->type]);
        ...
        XCopyArea(Dpy, pfishA[f->type][f->d], PixBuf, pgc, 0, 0,
                  rwidth[f->type], rheight[f->type], 0, 0);

        XCopyArea(Dpy, PixBuf, wid, gc, 0, 0, rwidth[f->type], rheight[f->type],
                  f->x, f->y);
      } else {
        XCopyArea(Dpy, pfishA[f->type][f->d], wid, gc, 0, 0, rwidth[f->type],
                  rheight[f->type], f->x, f->y);
      ...
      XPutImage(Dpy, wid, gc, xfishA[f->type][f->d], 0, 0, f->x, f->y,
    ...
    if (pfishB[f->type][f->d]) {
      if (DoClipping) {
        ...
        gcv.clip_y_origin = f->y;
        XChangeGC(Dpy, gc, GCClipMask | GCClipXOrigin | GCClipYOrigin, &gcv);
        XCopyArea(Dpy, pfishB[f->type][f->d], wid, gc, 0, 0, rwidth[f->type],
                  rheight[f->type], f->x, f->y);
      } else if (picname[0] != '\0') {
        gcv.fill_style = FillTiled;
        gcv.tile = PicMap;
        ...
        gcv.clip_y_origin = 0;
        XChangeGC(Dpy, pgc, GCClipMask | GCClipXOrigin | GCClipYOrigin, &gcv);
        XCopyArea(Dpy, pfishB[f->type][f->d], PixBuf, pgc, 0, 0,
                  rwidth[f->type], rheight[f->type], 0, 0);
      ...
      XPutImage(Dpy, wid, gc, xfishB[f->type][f->d], 0, 0, f->x, f->y,
                rwidth[f->type], rheight[f->type]);
    }
    f->frame = 1;
  }
...
int x, y, d;
{
  ...
  if (f->frame) {
    ...
    if (pfishA[f->type][f->d]) {
      ...
      if (DoClipping) {
        ...
        if (picname[0] != '\0') {
          ...
          XChangeGC(Dpy, pgc,
                    (GCFillStyle | GCTile | GCTileStipXOrigin |
                     GCTileStipYOrigin | GCClipMask | GCClipXOrigin |
                     GCClipYOrigin),
                    &gcv);
           ...
           */
          if (DoubleBuf) {
            XFillRectangle(Dpy, PixBuf, pgc, x - bx, y - by, rwidth[f->type],
                           rheight[f->type]);
            XFillRectangle(Dpy, ClipBuf, c0gc, 0, 0, 500, 500);
            XCopyArea(Dpy, mfishB[f->type][d], ClipBuf, cpgc, 0, 0,
                      rwidth[f->type], rheight[f->type], x - bx, y - by);
          } else {
            XFillRectangle(Dpy, wid, pgc, x, y, rwidth[f->type],
                           rheight[f->type]);
          }
        } else {
       ...
       */
      if (DoubleBuf) {
        XCopyArea(Dpy, pfishA[f->type][f->d], PixBuf, gc, 0, 0, rwidth[f->type],
                  rheight[f->type], f->x - bx, f->y - by);
        XCopyArea(Dpy, mfishA[f->type][f->d], ClipBuf, cpgc, 0, 0,
    ...
    if (pfishB[f->type][f->d]) {
        ...
        if (picname[0] != '\0') {
          ...
          if (DoubleBuf) {
            XFillRectangle(Dpy, PixBuf, pgc, x - bx, y - by, rwidth[f->type],
                           rheight[f->type]);
            XFillRectangle(Dpy, ClipBuf, c0gc, 0, 0, 500, 500);
            XCopyArea(Dpy, mfishA[f->type][d], ClipBuf, cpgc, 0, 0,
                      rwidth[f->type], rheight[f->type], x - bx, y - by);
          } else {
            XFillRectangle(Dpy, wid, pgc, x, y, rwidth[f->type],
                           rheight[f->type]);
      ...
      if (DoubleBuf) {
        XCopyArea(Dpy, pfishB[f->type][f->d], PixBuf, gc, 0, 0, rwidth[f->type],
                  rheight[f->type], f->x - bx, f->y - by);
        XCopyArea(Dpy, mfishB[f->type][f->d], ClipBuf, cpgc, 0, 0,
                  rwidth[f->type], rheight[f->type], f->x - bx, f->y - by);
        gcv.clip_mask = ClipBuf;
        gcv.clip_x_origin = bx;
        gcv.clip_y_origin = by;
...
void erasebubble(b, s) bubble *b;
int s;
{ XClearArea(Dpy, wid, b->x, b->y, s, s, 0); }

void putbubble(b, s, c) bubble *b;
int s;
unsigned long c;
{
  XGCValues gcv;
  ...
  XChangeGC(Dpy, bgc, GCForeground | GCClipMask | GCClipXOrigin | GCClipYOrigin,
            &gcv);
...
void FindColor(Dpy, colormap, colr) Display *Dpy;
Colormap colormap;
XColor *colr;
{
  int i, match;
...
int ColorUsage(data, width, height, colrs) unsigned char *data;
int width, height;
...
void init_colormap() {
  ...
  if (mlimit > 0) {
    ...
    for (j = 0; j < NUM_FISH; j++) {
      ...
      MedianCount((unsigned char *)xfishRasterA[j], (int)rwidth[j],
                  (int)rheight[j], cdp);
      free((char *)cdp);
    }
    MedianSplit(mlimit);
 ...
 */
XImage *MakeImage(data, width, height) unsigned char *data;
int width, height;
{
  ...
  *bitp = 0;
  ...
  newimage = XCreateImage(Dpy, DefaultVisual(Dpy, screen), depth, ZPixmap, 0,
                          (char *)bit_data, (width + linepad), height, 8,
                          bytesperline);
...
void init_pixmap() {
  ...
  for (k = 0; k < NUM_FISH; k++) {
    ...
    if ((DoClipping) || (picname[0] != '\0')) {
      ...
      mfishA[k][2] =
          XCreateBitmapFromData(Dpy, wid, (char *)data, rwidth[k], rheight[k]);
    ...
    if (pfishA[k][1]) {
      XPutImage(Dpy, pfishA[k][1], gc, xfishA[k][1], 0, 0, 0, 0, rwidth[k],
                rheight[k]);
    }
    if (pfishA[k][2]) {
      XPutImage(Dpy, pfishA[k][2], gc, xfishA[k][2], 0, 0, 0, 0, rwidth[k],
                rheight[k]);
    }
    if (pfishB[k][1]) {
      XPutImage(Dpy, pfishB[k][1], gc, xfishB[k][1], 0, 0, 0, 0, rwidth[k],
                rheight[k]);
    }
    if (pfishB[k][2]) {
      XPutImage(Dpy, pfishB[k][2], gc, xfishB[k][2], 0, 0, 0, 0, rwidth[k],
                rheight[k]);
    }
...
void set_window_type_desktop(Display *dpy, Window wid) {
  ...
  XChangeProperty(dpy, wid, _NET_WM_WINDOW_TYPE, XA_ATOM, 32, PropModeReplace,
...
void initialize() {
  ...
  if (picname[0] != '\0') {
    ...
    PicMap = XCreatePixmap(Dpy, root_window, Pwidth, Pheight,
  ...
  if (!DoClipping || picname[0] != '\0') {
    wid = XCreateWindow(Dpy, root_window, 1, 1, width - 2, height - 2, 0,
...
already gone.
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void collide_bubbles(f0, ofx, ofy) fish *f0;
int ofx, ofy;
{
  int i, delta;
  register fish *f = f0;
...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
int collide_fish(f0, xt) fish *f0;
int xt;
{
  int i, j;
  register fish *f = f0;
...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void high_res_sleep(seconds) double seconds;
{
